...
import { ArrowRight, User, ImageIcon, Clock } from "lucide-react";
...
export function PostCard({ post, index, viewMode }: PostCardProps) {
  ...
  return (
    <motion.div
      ...
      className="h-full"
    >
      <Card className={`group h-full overflow-hidden transition-all duration-300 hover:shadow-lg bg-card/80 hover:bg-card
        ${isList ? "md:flex" : ""} border border-border/40 hover:border-border/80`}
      >
        <Link href={`/blog/${slug}`} className={`${isList ? "md:flex md:flex-row" : "flex flex-col"} h-full`}>
          ...
          >
            ...
            <div className="absolute top-3 left-3 z-10">
              <Badge variant="secondary" className="bg-background/90 backdrop-blur-sm text-xs font-medium px-2 py-1 shadow-sm">
            ...
            {frontmatter.image ? (
              ...
              <div className="w-full h-full flex items-center justify-center p-4">
                <div className="text-center opacity-90">
                  <ImageIcon className="h-10 w-10 mx-auto mb-2 text-primary/40" strokeWidth={1.5} />
          ...
          <div className="flex flex-col h-full flex-grow">
            <CardHeader className="p-4 pb-2">
              ...
              <div className="mb-2 flex flex-wrap gap-1.5">
                {frontmatter.tags && frontmatter.tags.slice(0, isList ? 3 : 2).map((tag: string) => (
                  <Badge 
                    key={tag}
                    variant="outline" 
                ...
                {frontmatter.tags && frontmatter.tags.length > (isList ? 3 : 2) && (
                  <Badge 
                    variant="outline" 
              ...
              {/* Post description */}
              <CardDescription className={`text-sm mt-1.5 text-muted-foreground/90 ${isList ? "line-clamp-2" : "line-clamp-2"}`}>
