...
const components = {
  ...
  h1: ({ children, ...props }: React.ComponentPropsWithoutRef<'h1'>) => {
    ...
    return <h1 id={id} className="group flex mt-10 mb-5 scroll-mt-24" {...props}>{children}</h1>;
  },
  h2: ({ children, ...props }: React.ComponentPropsWithoutRef<'h2'>) => {
    const text = extractTextFromChildren(children);
    const id = text.toLowerCase().replace(/\s+/g, '-').replace(/[^\w-]/g, '');
    console.log("h2 text:", text, "id:", id);
    return <h2 id={id} className="group flex mt-10 mb-5 scroll-mt-24" {...props}>{children}</h2>;
  },
  h3: ({ children, ...props }: React.ComponentPropsWithoutRef<'h3'>) => {
    const text = extractTextFromChildren(children);
    const id = text.toLowerCase().replace(/\s+/g, '-').replace(/[^\w-]/g, '');
    console.log("h3 text:", text, "id:", id);
    return <h3 id={id} className="group flex mt-8 mb-4 scroll-mt-24" {...props}>{children}</h3>;
  },
  h4: ({ children, ...props }: React.ComponentPropsWithoutRef<'h4'>) => {
    const text = extractTextFromChildren(children);
    const id = text.toLowerCase().replace(/\s+/g, '-').replace(/[^\w-]/g, '');
    return <h4 id={id} className="group flex mt-6 mb-3 scroll-mt-24" {...props}>{children}</h4>;
  },
  h5: ({ children, ...props }: React.ComponentPropsWithoutRef<'h5'>) => {
    const text = extractTextFromChildren(children);
    const id = text.toLowerCase().replace(/\s+/g, '-').replace(/[^\w-]/g, '');
    return <h5 id={id} className="group flex mt-4 mb-2 scroll-mt-24" {...props}>{children}</h5>;
  },
